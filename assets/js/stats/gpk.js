var app = angular.module("dsm.stats.gpk", [
...
// StatsGPKGroupController
// Lists session groups and perfor search and order
app.controller("StatsGPKGroupController", function($scope, gatewaySocket, Restangular, $uibModal, $cookies) {
	$scope.store = {
	...
	function reload(){
		Restangular.one('/sessions_gpk/info').get().then(function(info) {
			$scope.totalItems = info;
		});

		Restangular.all('/sessions_gpk').getList({
	...
	$scope.editEntry = function(group){
		var modalInstance = $uibModal.open({
			...
			controller: 'StatsGPKGroupEditController',
			backdrop: 'static',
			keyboard: false,
			size: "lg",
			resolve: {
...
// StatsGPKGroupEditController
// Displays an overlay to edit group object
app.controller('StatsGPKGroupEditController', function (Restangular, $scope, $uibModalInstance, group, gatewaySocket) {
