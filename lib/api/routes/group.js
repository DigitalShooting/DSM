...
module.exports = function(mysql, config){
	...
	group.get = function create(req, res) {
		...
		mysql.query(
			"SELECT sessionGroup.id, sessionGroup.unixtime, sessionGroup.line, sessionGroup.disziplin, sessionGroup.note, user.firstName, user.lastName, verein.name as 'verein', verein.id as 'vereinID'" +
			"FROM sessionGroup " +
