module.exports = function(mysql, config){
	...
	rwk.id.gast.get = function create(req, res, next) {
		getShotIn(req.params.id, "gast", function(rows){
			res.send(201, rows);
		});
	}

	rwk.id.heim.get = function create(req, res, next) {
		getShotIn(req.params.id, "heim", function(rows){
			res.send(201, rows);
		});
	}
