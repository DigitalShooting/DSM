module.exports = function(mysql){
	...
	manschaft.get = function create(req, res) {
		...
		if (req.query.search !== undefined){
			...
			for (var i in strings){
				sql += "AND CONCAT_WS('|', manschaft.name, manschaft.id, manschaft.note, manschaft.anzahlSchuetzen) LIKE "+mysql.escape("%"+strings[i]+"%")+" COLLATE utf8_general_ci ";
			}
			query.searchSQL = sql;
		}
		...
		mysql.query(
			"SELECT manschaft.name, manschaft.id, manschaft.note, manschaft.anzahlSchuetzen " +
			"FROM manschaft " +
			"WHERE 1 = 1 " +
			query.searchSQL +
	...
	manschaft.manschaftID.post = function create(req, res) {
		mysql.query(
			"UPDATE manschaft " +
			"SET manschaft.name = ?, manschaft.note = ?, manschaft.anzahlSchuetzen = ? " +
			"WHERE manschaft.id = ?; ",
			[req.body.name, req.body.note, req.body.anzahlSchuetzen, req.params.manschaftID],
			function(err, rows) {
				return res.send(201, rows);
			}
		);
