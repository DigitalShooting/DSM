module.exports = function(mysql, config){
	...
	saison.saisonID.post = function create(req, res) {
		mysql.query(
			"UPDATE saison " +
			"SET saison.name = ?, saison.note = ?, saison.disziplinID = ?" +
			"WHERE saison.id = ?; ",
			[req.body.name, req.body.note, req.body.disziplinID, req.params.saisonID],
			function(err, rows) {
				return res.send(201, rows);
			}
		);
	...
	function getSaison(id, callback){
		mysql.query(
			"SELECT saison.id, saison.name, saison.note, saison.disziplinID " +
			"FROM saison " +
			"WHERE saison.id = ? ",
			[id],
			function(err, rows) {
